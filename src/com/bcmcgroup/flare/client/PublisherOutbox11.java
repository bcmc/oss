...
/**
 * Outbox class for publishing TAXII 1.1 messages
 * @version 2.0.3
 */
class PublisherOutbox11 {

    private static final Logger logger = LogManager.getLogger(PublisherOutbox11.class);
    ...
    private static final String taxii11serverUrlInbox;
    ...
    static {
        ...
        if ((taxii11serverUrlInbox = config.getProperty("taxii11serverUrlInbox")) == null || taxii11serverUrlInbox.isEmpty())
            logger.warn("taxii11serverUrlInbox is not set, or is not being read properly.");
     ...
     *
     * @param collectionName the collection name to be published to.
     ...
     */
    private static int publish(String collectionName, HashMap<String, String> extHdrs, String message, List<ContentBlock> contentBlocks) throws IOException, SAXException {

        Document taxiiDoc = TaxiiUtil.inboxMessage(null, null, extHdrs, collectionName, message, null, null, contentBlocks);
        ...
        if (connectingToFLARE.equals("true")) {
            address = taxii11serverUrlInbox + "/" + URLEncoder.encode(collectionName, "UTF-8").replace("+", "%20");
        } else {
            address = taxii11serverUrlInbox;
        }
